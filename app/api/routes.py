...
def create_role():
    ...
    if default is None:
        ...
        u = user_datastore.create_user(email="test@refns.com",
                                       ...
                                       name="chewie_admin",
                                       username="chewie_admin",
                                       organization="UMMI")
        # create Contributor user
        u2 = user_datastore.create_user(email="chewie@kessel.com",
                                        password="whereishan",
                                        name="chewie",
                                        username="chewie",
                                        organization="UMMI")
        ...
        user_datastore.add_role_to_user(u2, "Contributor")
        ...
        # check if Contributor was successfully created in Postgres db
        postgres_contributor = user_datastore.get_user(2)
        postgres_contributor_account = False if postgres_contributor is None else True
        ...
        # Create Contributor in Virtuoso
        new_user_url2 = '{0}users/{1}'.format(current_app.config['BASE_URL'],
                                             str(u2.id))
        newUserRole2 = 'Contributor'
        sparql_query2 = (sq.INSERT_USER.format(current_app.config['DEFAULTHGRAPH'],
                                                          new_user_url2,
                                                          newUserRole2))

        result2 = aux.send_data(sparql_query2,
                               current_app.config['LOCAL_SPARQL'],
                               current_app.config['VIRTUOSO_USER'],
                               current_app.config['VIRTUOSO_PASS'])

        # check if Admin was successfully inserted into Virtuoso graph
        virtuoso_contributor = result2.status_code
        virtuoso_contributor_account = True if virtuoso_contributor in [200, 201] else False

        print('Created Postgres Contributor: {0}\n'
              'Created Virtuoso Contributor: {1}'.format(postgres_contributor_account, virtuoso_contributor_account), flush=True)
...
class SchemaListAPItypon(Resource):
    ...
    def post(self, species_id):
        ...
        # determine if schema hashes are from one of the tutorial schemas
        tutorial_file = current_app.config['TUTORIAL_HASHES']
        ...
        for sp, sc in tutorial_hashes.items():
            hashes = sc
            if set(hashes) == set(schema_hashes):
                accepted = True

        if accepted is False:
            return {'Unauthorized': 'Schema is not a tutorial schema.'}, 403
